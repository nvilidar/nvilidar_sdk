...
namespace nvilidar
{
	...
	class  NVILIDAR_API LidarDriverSerialport
    {
		public:
			...
			bool LidarInit();					//雷达初始化 获取参数配置参数等 
			bool LidarCloseHandle();			//雷达断开连接  
			bool LidarStartScan();				//启动扫描  
			bool LidarStopScan();				//停止扫描
			...
			bool SetIntensities(const uint8_t has_intensity, uint32_t timeout = DEFAULT_TIMEOUT);     //设置是否有信号质量
			bool GetDeviceInfo(Nvilidar_DeviceInfo & info, uint32_t timeout = DEFAULT_TIMEOUT);
			...
			//保存参数
			bool SaveCfg(bool &flag,uint32_t timeout = DEFAULT_TIMEOUT);

			//最终输出数据  一圈点云的数据  
			bool waitCircleResponse(CircleDataInfoTypeDef &info, uint32_t timeout = DEFAULT_TIMEOUT);
		...
		private:
			...
			bool waitNormalResponse(uint32_t timeout = DEFAULT_TIMEOUT);	//等待雷达应答 
